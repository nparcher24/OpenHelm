...
- **Dual-WitMotion Heading Fusion + Three-Way GPS Arbitration** (2026-05-15, on dev — user is about to mount the second unit and deploy): User has two WitMotion AHRS units + the Garmin N2K GPS. Mag-only heading is unusable due to console interference. Built (a) a stateful complementary filter that fuses dual gyros + COG, using the secondary unit's mag as a gradiometer trust-detector, and (b) three-way GPS position arbitration (primary, secondary, n2k) selecting tightest HDOP with tiebreak primary > secondary > n2k. Secondary unit is opt-in via `OPENHELM_GPS_SECONDARY=1` so today's single-unit boat works unchanged. Tests: 50 across fusion (36) + arbiter (44). **Trigger phrase "get the heading working" → read `HEADING_FUSION.md` and start at §0 (verify the secondary's signal is arriving) BEFORE doing anything else.** User mounts hardware before invoking. GPS page shows a "GPS Sources" panel with all three providers; active one is highlighted.
