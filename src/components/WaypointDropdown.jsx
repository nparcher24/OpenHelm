...
export default function WaypointDropdown({
  waypoints,
  driftCorrections, // { [waypointId]: { lat, lng, approximate, offsetM } }
  onSelect,
  onClose
}) {
  return (
      ...
      <div className="absolute top-14 right-0 bg-terminal-surface rounded-lg shadow-glow-green border-2 border-terminal-green z-40 min-w-[280px] max-h-[400px] overflow-hidden">
        ...
        <div className="max-h-[340px] overflow-y-auto">
          {waypoints.length === 0 ? (
            ...
            waypoints.map(wp => {
              const dc = driftCorrections?.[wp.id]
              return (
                <button
                  key={wp.id}
                  onClick={() => onSelect(wp)}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {dc && (
                      <div className="text-terminal-cyan text-xs font-mono truncate">
                        DRIFT → {formatCoordinates(dc.lat, dc.lng)}
                        {dc.approximate ? ' ~' : ''}
                      </div>
                    )}
                    {wp.description && (
