 ...
 * GeoJSON served directly to MapLibre (no tippecanoe/Martin needed)
 */
...
export async function getStorageInfo() {
  try {
    ...
    // Scan for region directories (each contains GeoJSON files)
    let entries = [];
    try { entries = await fs.readdir(S57_DIR, { withFileTypes: true }); } catch { entries = []; }
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        const regionDir = path.join(S57_DIR, entry.name);
        try {
          const metaPath = path.join(regionDir, 'metadata.json');
          await fs.access(metaPath);
          downloadedCount++;
          // Sum up GeoJSON file sizes
          const files = await fs.readdir(regionDir);
          for (const f of files) {
            if (f.endsWith('.geojson')) {
              const stats = await fs.stat(path.join(regionDir, f));
              totalSizeMB += stats.size / 1024 / 1024;
            }
          }
        } catch { /* not a valid region dir */ }
      }
    }
...
/**
 * Get downloaded regions - looks for directories containing metadata.json
 */
export async function getDownloadedRegions() {
  try {
    await fs.mkdir(S57_DIR, { recursive: true });
    const entries = await fs.readdir(S57_DIR, { withFileTypes: true });
    const regions = [];
    ...
    for (const entry of entries) {
      ...
      const regionDir = path.join(S57_DIR, regionId);
      const metaPath = path.join(regionDir, 'metadata.json');
      ...
      try {
        ...
        for (const f of files) {
          if (f.endsWith('.geojson')) {
            const stats = await fs.stat(path.join(regionDir, f));
            totalSizeMB += stats.size / 1024 / 1024;
            layerFiles.push(f.replace('.geojson', ''));
          }
        }
        ...
        regions.push({
          ...
          sizeMB: parseFloat(totalSizeMB.toFixed(1)),
        ...
        // Not a valid region directory, skip
      }
    }
...
export async function getRegionLayers(regionId) {
  ...
  try {
    ...
    for (const f of files) {
      if (f.endsWith('.geojson')) {
        const layerName = f.replace('.geojson', '');
        const stats = await fs.stat(path.join(regionDir, f));
        layers.push({
          name: layerName,
          sizeMB: parseFloat((stats.size / 1024 / 1024).toFixed(1)),
          url: `/tiles/s57/${regionId}/${f}`
        });
      }
    }
...
async function extractS57Layers(s57File, outputDir, signal) {
  ...
  for (const layer of TARGET_LAYERS) {
    ...
    try {
      ...
      if (stats.size > 50) {
        extracted.push({ layer, file: outputFile });
      } else {
...
/**
 * Main processing job - download, extract, convert to GeoJSON (no tippecanoe)
 */
async function processS57Job(jobId, regionId, signal) {
  ...
  try {
    ...
    // Phase 3: Extract layers to GeoJSON (35-85%)
    console.log(`[S57] Phase 3: Converting S-57 layers to GeoJSON...`);
    global.broadcastProgress(jobId, 35, 'converting', 'Extracting chart layers...', null);

    const allExtracted = [];
    for (let i = 0; i < s57Files.length; i++) {
      if (signal?.aborted) throw new Error('Cancelled');

      const progress = 35 + ((i / s57Files.length) * 50);
      global.broadcastProgress(
        jobId, progress, 'converting',
        `Extracting layers from chart ${i + 1}/${s57Files.length}: ${path.basename(s57Files[i])}`,
        null
      );

      const extracted = await extractS57Layers(s57Files[i], tempGeojsonDir, signal);
    ...
    // Phase 4: Merge GeoJSON by layer and save to final location (85-95%)
    global.broadcastProgress(jobId, 85, 'converting', 'Merging layers...', null);
    ...
    global.broadcastProgress(jobId, 95, 'finalizing', 'Saving metadata...', null);
    ...
    await fs.writeFile(path.join(finalDir, 'metadata.json'), JSON.stringify(metadata, null, 2));
